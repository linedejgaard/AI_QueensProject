...
public class QueensLogic13 implements IQueensLogic {
        ...
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                if (board[col][row] == 0) {
                    ...
                    BDD pos = rules.restrict(fact.ithVar(variable)); // assign the variable to true
                    BDD neg = rules.restrict(fact.nithVar(getVariable(col, row))); // assign the variable to false
                    if (neg.isZero()) { // not inserting a queen here is a contradiction
                        board[col][row] = 1;
                        insertQueen(col, row); // insert the rest of the queens
    ...
    // RULES: //https://homes.cs.aau.dk/~srba/courses/SV-08/material/09.pdf
    // (exclusion of variables and one queen in each row)
    // excludedRule: no other queen can be positioned on the same row, column or any
    // of the diagonals
    // oneQueenInEachRowRule: at least one queen in each row: conjunction of
    // disjunctions
    // NOTES:
    // The BDD must be true in the end if the queens are placed correctly
    // Placing a queen makes the variable true
    // excludedRule: conjucntion of negations, since placing a queen on one of the
    // excluded positions will make the conjunction false, and hence we do not end
    // up having a true BDD
    // oneQueenInEachRowRule: disjunction since at least one variable in the
    // disjunction must be true to make the disjunction true, and that is placing a
    // queen in at least one row, since there is a disjunction for each row.
    public void buildRules() { // INIT RULES
        ...
        // rule: no other queen can be positioned on the same row, column or any of the
        // diagonals (https://homes.cs.aau.dk/~srba/courses/SV-08/material/09.pdf)
        for (int variable = 0; variable < nVars; variable++) {
            rules = rules.and(excludedRule(variable));// rules = rules.and(excludedRule(variable)); //IS THE SAME
                                                      // rules.apply(excludedRule(variable), BDDFactory.and);
        }

        // rule: must be at least one queen in each row
        // (https://homes.cs.aau.dk/~srba/courses/SV-08/material/09.pdf)
        rules = rules.and(oneQueenInEachRowRule());// rules = rules.and(oneQueenInEachRowRule()); //IS THE SAME
                                                   // rules.apply(oneQueenInEachRowRule(), BDDFactory.and);
    }

    // https://homes.cs.aau.dk/~srba/courses/SV-08/material/09.pdf
    public BDD oneQueenInEachRowRule() { // disjunction: this means that there must be at least ONE queen in each row to
                                         // make the BDD true
        ...
        for (int row = 0; row < size; row++) { // run through the rows and make a rowRule for each
            ...
            for (int col = 0; col < size; col++) { // run through the cols and make the disjunction of the positions in
                                                   // the row: x1 OR x2 OR x3 etc.
                rowRule = rowRule.or(fact.ithVar(getVariable(col, row)));// IS THE SAME
                                                                         // apply(fact.ithVar(getVariable(col, row)),
                                                                         // BDDFactory.or);
            }
            rule = rule.and(rowRule); // IS THE SAME apply(rowRule, BDDFactory.and); //conjoin the individual row
                                      // rules such that row1Rule AND row2Rule AND row3Rule etc.
        }
        return rule;
    }
    ...
    public BDD excludedRule(int variable) { // conjunction
        BDD rule = True; // The conjunctive identity is true
                         // //https://en.wikipedia.org/wiki/Logical_conjunction
        for (int excludedVariable : getExcludedVariablesBy(variable)) {
            rule = rule.and(fact.nithVar(excludedVariable)); // IS THE SAME apply(fact.nithVar(excludedVariable),
                                                             // BDDFactory.and); //negate all the excluded variables
                                                             // meaning that the excluded variables must be false to
                                                             // make the conjunction true
        }
        // if the variable is true (the queen is on the position/variable) the excluded
        // variables must be negated
    ...
    private int getRow(int variable) {
        // return variable/size; //I don't know if this gives rows. It depends on how
        // divid works on ints, I think...
